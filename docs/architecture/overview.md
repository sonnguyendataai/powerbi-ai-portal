...
- Report creation: Chart Studio with `create=true` builds a **PBIR** report definition (`pbir-builder.ts`) bound to the scoped semantic model via a `byConnection` reference, and creates it with the **Fabric** Create Report API (`api.fabric.microsoft.com`, separate `fetchFabricToken`). Handles the 202 long-running operation. Requires Fabric/Premium/PPU capacity and the service principal as a workspace Contributor; otherwise the Fabric error is surfaced verbatim. The Power BI REST API has no create-with-layout operation, so this path is required.
